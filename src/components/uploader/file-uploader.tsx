        ...
        rejectedFiles.forEach(({ file, errors }) => {
          errors.forEach((error) => {
            if (error.code === "file-too-large") {
              toast.error(
                `File ${file.name} exceeds the size limit of ${formatBytes(maxSize)}`
              )
        ...
        <DialogContent className="sm:max-w-3xl">
          ...
          <div className="flex flex-col gap-4">
            ...
            <div className="relative h-96 w-full">
              ...
              imagesToCrop[currentCropIndex]?.preview ? (
                <>
                  {console.log(
                    `Rendering Cropper for image: ${imagesToCrop[currentCropIndex].name}`
                  )}
                  <Cropper
